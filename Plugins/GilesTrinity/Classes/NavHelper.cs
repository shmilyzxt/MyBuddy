...
namespace GilesTrinity
{
    class NavHelper
    {
        ...
        internal static void UpdateSearchGridProvider(bool force = false)
        {
            ...
            if (GilesTrinity.Settings.Combat.Misc.UseNavMeshTargeting || force)
            {
                if (ZetaDia.IsInGame)
                {
        ...
        internal static bool CanRayCast(Vector3 vStartLocation, Vector3 vDestination, float ZDiff = 4f)
        {
            ...
            if (!rc)
            {
                if (GilesTrinity.hashNavigationObstacleCache.Any(o => MathEx.IntersectsPath(o.Location, o.Radius, vStartLocation, vDestination)))
                    return false;
                else
                    return true;
            }
